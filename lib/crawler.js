  ...
  try {
    while (queue.length && visited.size < maxPages && products.length < maxProducts) {
      ...
      try {
        ...
        const links = uniqueLinks(snapshot.links, snapshot.url).filter(link => link.score >= 2).sort((a, b) => b.score - a.score).slice(0, 120);
        let added = 0;
    ...
    const status = blocked ? 'blocked' : products.length ? 'completed' : 'no_results';
    return { status, pages: visited.size, products, elapsedMs: Date.now() - started, queueRemaining: queue.length };
  } finally {
    await context.close().catch(() => {});
  }
}

module.exports = { BLOCKED_RE, IGNORE_LINK, scoreLink, uniqueLinks, progressiveScroll, crawl };
