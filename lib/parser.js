...
const PRODUCT_HINTS = /\/products?\/|\/produto\/|\/p\/|product[-_]?id|sku=/i;
const CATEGORY_HINTS = /\/categor(?:y|ia)s?\/|\/collections?\/|\/cole(?:cao|coes|ção|ções)\/|\/departamentos?\/|\/t\/produtos/i;
...
function parseSnapshot(snapshot, supplier, sourceCategory = '') {
  ...
  const evidence = Number(Boolean(structured)) * 4 + Number(Boolean(name)) * 2 + Number(Boolean(price)) * 3 + Number(PRODUCT_HINTS.test(snapshot.url));
  const categoryOnly = CATEGORY_HINTS.test(snapshot.url) && !structured && !price;
...
module.exports = { CATEGORY_HINTS, PRODUCT_HINTS, cleanText, parseMoney, packagingType, normalizeImage, findProductJsonLd, parseSnapshot };
